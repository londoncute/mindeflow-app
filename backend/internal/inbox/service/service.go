...
func (s *Service) Create(ctx context.Context, userID int64, input inbox.CreateInput) (inbox.InboxItem, error) {
	if input.Title == "" {
...
func (s *Service) List(ctx context.Context, userID int64, filter inbox.ListFilter) (inbox.ListResult, error) {
	if filter.Limit <= 0 {
		filter.Limit = 20
	}
	if filter.Offset < 0 {
...
func (s *Service) Skip(ctx context.Context, userID int64, id int) (inbox.SkipResult, error) {
	...
	item, err = s.repo.Skip(ctx, userID, id)
	if err != nil {
		if err == repository.ErrInboxItemNotFound {
			return inbox.SkipResult{}, utils.NewNotFound("inbox item not found")
		}
